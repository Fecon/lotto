...
class Form_pure_seed_model extends CI_Model {
	...
	{
		$this->db->insert('form_pure_seed',$input);
	...
	{
		$this->db->insert('form_pure_seed_repeat',$input);
	}
	
	public function get_data($research_id,$pedigree_id)
	{

		$this->db->where('form_pure_seed.research_id',$research_id)
				->where('form_pure_seed.pedigree_id',$pedigree_id);
			//->join('research_pedigree','research_pedigree.rp_id = research_pedigree_plot.rp_id');
		$query1 = $this->db->get('form_pure_seed');
		$query = $query1->result_array(); 
		
		foreach ($query as $key => $value) {
			$this->db->where('pure_seed_id',$value['id']);
				// ->group_by('plot_no') // BUG //
			$query2 = $this->db->get('form_pure_seed_repeat');
			$query[$key]['pure_seed_repeat'] = $query2->result_array(); 
		}
		return $query ;
	}
	...
	public function update_repeat_data($input)
	{
		...
		$this->db->where('repeat_id',$input['repeat_id'])
			->update('form_pure_seed_repeat',$input);
		////////// End Operation //////////
		$this->db->trans_complete();
		// was there any update or error?
		if ($this->db->affected_rows() == '1') {
	...
	{
		$this->db->trans_start();
		$this->db->where('pure_seed_id',$id)
			->delete('form_pure_seed_repeat');
		$this->db->trans_complete();
	}
