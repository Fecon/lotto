...
class Form_product_model extends CI_Model {
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_product',$input);
	...
	public function get_data($research_id,$repeat)
	{
		...
		$query = $this->db->get('form_product');
		return $query->result_array(); 
	}
	...
	public function get_lost_grove($research_id,$repeat)
	{
		$this->db->where('research_id',$research_id)
			->where('repeatedly',$repeat)
			->order_by('plot_no','asc');
