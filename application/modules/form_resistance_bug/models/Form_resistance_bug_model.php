...
class Form_resistance_bug_model extends CI_Model {
	...
	public function get_resistance_bug_detail($id)
	{
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_resistance_bug',$input);
	...
	public function update_data($input)
	{
		...
		$this->db->where('id',$input['id'])
			->update('form_resistance_bug',$input);
		$this->db->trans_complete();
		// was there any update or error?
		if ($this->db->affected_rows() == '1') {
		    return TRUE;
	...
	public function get_data($research_id,$resistance_id)
	{
		$this->db->where('research_id',$research_id)
			->where('resistance_type_id',$resistance_id)
			->order_by('entry,id','asc')
			->join('pedigree','pedigree.pedigree_id = form_resistance_bug.pedigree_id');
		$query = $this->db->get('form_resistance_bug');
		return $query->result_array(); 
	}
	public function check_data($research_id,$resistance_id)
	{

		$query = $this->db->where('research_id',$research_id)
			->where('resistance_type_id',$resistance_id)
			->get('form_resistance_bug',1);
	...
	public function clear_data($research_id,$resistance_type_id)
	{
		$this->db->trans_start();
		$this->db->where('research_id',$research_id)
			->where('resistance_type_id',$resistance_type_id)
			->delete('form_resistance_bug');
		$this->db->trans_complete();
	}
}
