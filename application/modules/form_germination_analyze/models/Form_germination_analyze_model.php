...
class Form_germination_analyze_model extends CI_Model {
	...
	{
		$this->db->insert('form_germination_analyze',$input);
	...
	{
		$this->db->insert('form_germination_analyze_repeat',$input);
	}
	
	public function get_data($research_id,$pedigree_id)
	{

		$this->db->where('form_germination_analyze.research_id',$research_id)
				->where('form_germination_analyze.pedigree_id',$pedigree_id);
			//->join('research_pedigree','research_pedigree.rp_id = research_pedigree_plot.rp_id');
		$query1 = $this->db->get('form_germination_analyze');
		$query = $query1->result_array(); 
		
		foreach ($query as $key => $value) {
			$this->db->where('analyze_id',$value['id']);
				// ->group_by('plot_no') // BUG //
			$query2 = $this->db->get('form_germination_analyze_repeat');
	...
	public function update_data($input)
	{
		...
		$this->db->where('id',$input['id'])
			->update('form_germination_analyze',$input);
		////////// End Operation //////////
	...
	public function update_repeat_data($input)
	{
		...
		$this->db->where('repeat_id',$input['repeat_id'])
			->update('form_germination_analyze_repeat',$input);
		////////// End Operation //////////
		$this->db->trans_complete();
		// was there any update or error?
		if ($this->db->affected_rows() == '1') {
	...
	{
		$this->db->trans_start();
		$this->db->where('analyze_id',$id)
			->delete('form_germination_analyze_repeat');
		$this->db->trans_complete();
	}
	public function check_data($research_id,$pedigree_id)
	{
		$query = $this->db->where('research_id',$research_id)
			...
			->get('form_germination_analyze',1);
