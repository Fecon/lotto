...
class Form_determinant_model extends CI_Model {
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_determinant',$input);
	...
	public function insert_filename($input)
	{
		...
		$this->db->trans_start();
		$this->db->insert('form_determinant_attachment',$input);
		$this->db->trans_complete();
	...
	public function get_data($research_id)
	{
		$this->db->where('research_id',$research_id)
			->join('user','user.user_id=form_determinant.user_id');
		$query1 = $this->db->get('form_determinant');
		$query = $query1->result_array();

		foreach ($query as $key => $value) {
			$this->db->where('record_id',$value['id']);
			$query2 = $this->db->get('form_determinant_attachment');
