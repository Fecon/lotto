...
class Form_1000_weight_model extends CI_Model {
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_1000_weight',$input);
	...
	public function get_data($research_id)
	{
		$this->db->where('research_id',$research_id)
			->order_by('entry','asc');
		$query = $this->db->get('form_1000_weight');
		return $query->result_array(); 
	}
	public function check_data($research_id)
	{
		...
		$query = $this->db->where('research_id',$research_id)
			->get('form_1000_weight',1);
		return $query->num_rows(); 
	}
	public function get_research_pedigree($research_id)
	{
		$this->db->where('research_pedigree.research_id',$research_id)
