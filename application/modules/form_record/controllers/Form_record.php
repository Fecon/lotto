...
class Form_record extends MX_Controller {

	public function __construct()
    {
    	parent::__construct();
    	$this->load->model('Form_record_model');
    }
	public function index()
	{
		$research_id = $this->uri->segment(3);
		$data['research_id'] = $research_id ;
		$data['get_data'] = $this->Form_record_model->get_data($research_id);
	...
	public function record_edit()
	{
		...
		$data['get_data'] = $this->Form_record_model->get_data($research_id);
		$data['get_detail'] = $this->Form_record_model->get_detail($record_id);
		// echo "<pre>";
	...
	public function record_insert()
	{
		...
		$record_id = $this->Form_record_model->insert_data($list_data);
		...
		if(!empty($check_img['tmp_name'][0])){
			for ($i=0; $i < count($images_name) ; $i++) { 
				...
				$resultfile = $this->Form_record_model->insert_filename($input_file);
			}
		}
		...
		redirect('form_record/record_list/'.$this->input->post('research_id'));
	}
	public function record_update()
	{
		...
		$this->Form_record_model->update_data($list_data);
	...
	public function img_delete()
	{
		...
		$research_id = $this->uri->segment(3);
		$record_id = $this->uri->segment(4);
		$this->Form_record_model->delete_attachment($file_id);
		redirect('form_record/record_edit/'.$research_id.'/'.$record_id);
	}
	public function do_upload($images)
	{
		$config['upload_path'] = './assets/files/records/';
