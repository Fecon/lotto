...
class Form_tree_grove extends MX_Controller {

	public function __construct()
    {
      	parent::__construct();
       	$this->load->model('Form_tree_grove_model');
       	$this->load->model('form_comment/Comment_model');
    }
	public function index()
	{
		$data['content'] = 'form_template/form-template';
		$data['header_title'] = 'จำนวนต้นต่อกอ';
		...
		$data['form_content'] = 'form_tree_grove/form_tree_grove';

		$research_id = $this->uri->segment(3);
		$data['research_id'] = $research_id ;
		
		$check_data = $this->Form_tree_grove_model->check_data($research_id);
		if($check_data!=0){
			redirect('form_tree_grove/edit_view/'.$research_id);
		}

		$data['get_research'] = $this->Form_tree_grove_model->get_research_detail($research_id);
		for ($repeat=1; $repeat <= $data['get_research']['repeat_amount']; $repeat++) { 
			$data['pedigree_plot'][] = $this->Form_tree_grove_model->get_pedigree_plot_repeat($research_id,$repeat);
		}
	...
	public function edit_view()
	{
		...
		$data['form_submit'] = 'form_template/form-submit';
		$data['form_comment'] = 'form_template/form-comment';
		$data['form_content'] = 'form_tree_grove/form_tree_grove_edit';
		...
		for ($repeat=1; $repeat <= $data['get_research']['repeat_amount']; $repeat++) { 
			$data['pedigree_plot'][] = $this->Form_tree_grove_model->get_pedigree_plot_repeat($research_id,$repeat);
			$data['get_data'][] = $this->Form_tree_grove_model->get_data($research_id,$repeat);
		}
	...
	public function insert_data()
	{
		...
		for ($i=0; $i < count($plot_no); $i++) {
			...
			// echo "<Pre>";
			// print_r($input);
			$this->Form_tree_grove_model->insert_data($input);
		}
	...
	public function update_data()
	{
		...
		for ($i=0; $i < count($id); $i++) { 
			...
			$this->Form_tree_grove_model->update_data($input);
		}
