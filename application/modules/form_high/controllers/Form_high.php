...
class Form_high extends MX_Controller {

	public function __construct()
    {
      	parent::__construct();
       	$this->load->model('Form_high_model');
       	$this->load->model('form_comment/Comment_model');
    }
	public function index()
	{
		$data['content'] = 'form_template/form-template';
		$data['header_title'] = 'ความสูง';
		...
		$data['form_content'] = 'form_high/form_high';
		...
		$check_data = $this->Form_high_model->check_data($research_id);
		if($check_data!=0){
			redirect('form_high/edit_view/'.$research_id);
		}

		$data['get_research'] = $this->Form_high_model->get_research_detail($research_id);
		for ($repeat=1; $repeat <= $data['get_research']['repeat_amount']; $repeat++) { 
			$data['pedigree_plot'][] = $this->Form_high_model->get_pedigree_plot_repeat($research_id,$repeat);
		}
	...
	public function edit_view()
	{
		...
		$data['form_submit'] = 'form_template/form-submit';
		$data['form_comment'] = 'form_template/form-comment';
		$data['form_content'] = 'form_high/form_high_edit';
		...
		for ($repeat=1; $repeat <= $data['get_research']['repeat_amount']; $repeat++) { 
			$data['pedigree_plot'][] = $this->Form_high_model->get_pedigree_plot_repeat($research_id,$repeat);
			$data['get_data'][] = $this->Form_high_model->get_data($research_id,$repeat);
		}
	...
	public function insert_data()
	{
		...
		for ($i=0; $i < count($plot_no); $i++) {
			...
			// echo "<Pre>";
			// print_r($input);
			$this->Form_high_model->insert_data($input);
		}
		$input	= array(
			'research_id' => $research_id, 
	...
	public function update_data()
	{
		...
		for ($i=0; $i < count($id); $i++) { 
			...
			$this->Form_high_model->update_data($input);
		}
		$input	= array(
			'research_id' => $research_id, 
			'form_type' => $this->uri->segment(1), 
