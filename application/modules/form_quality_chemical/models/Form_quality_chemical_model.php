...
class Form_quality_chemical_model extends CI_Model {
	...
	public function get_form_quality_chemical()
	{
		$query = $this->db->get('form_quality_chemical');
		return $query->result_array(); 
	}
	public function get_form_quality_chemical_detail($id)
	{
		$query = $this->db->where('id',$id)
			->get('form_quality_chemical');
		return $query->result_array();
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_quality_chemical',$input);
	...
	public function update_data($input)
	{
		...
		$this->db->where('id',$input['id'])
			->update('form_quality_chemical',$input);
		$this->db->trans_complete();
		// was there any update or error?
		if ($this->db->affected_rows() == '1') {
		    return TRUE;
	...
	public function get_data($research_id)
	{
		$this->db->where('research_id',$research_id)
			...
			->join('pedigree','pedigree.pedigree_id = form_quality_chemical.pedigree_id');
		$query = $this->db->get('form_quality_chemical');
		return $query->result_array(); 
	}
