  ...
  <section class="content-header">
    ...
    <div align="right">
      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalMd"><i
          class="fa fa-plus"></i> เพิ่มงวด</button>
    </div>
  </section>
                  ...
                  <table class="table table-striped">
                    ...
                    <tr>
                      <td class="text-center">
                        <div class="col-xs-9">
                          <input name="name" class="form-control input-lg" type="date"
                            value="<?php echo $lotto['name'] ?>" onkeypress="return isNumber(event)"
                            onclick="show_icon('<?php echo 'name_'.$lotto['id'] ?>')"
                            onfocusout="hide_icon('<?php echo 'name_'.$lotto['id'] ?>')">
                        </div>
                        <div class="col-xs-3">
                          <button id="<?php echo 'name_'.$lotto['id'] ?>" class="btn btn-success btn-lg hide-icon"
                            type="submit"><i class="fa fa-check" aria-hidden="true"></i></button>
                        </div>
                      </td>
                      <td class="text-center">
                        <div class="col-xs-9">
                          <input name="2top" class="form-control input-lg" type="text" maxlength="2"
                            value="<?php echo $lotto['2top'] ?>" onkeypress="return isNumber(event)"
                            onclick="show_icon('<?php echo '2top_'.$lotto['id'] ?>')"
                            onfocusout="hide_icon('<?php echo '2top_'.$lotto['id'] ?>')">
                        </div>
                        <div class="col-xs-3">
                          <button id="<?php echo '2top_'.$lotto['id'] ?>" class="btn btn-success btn-lg hide-icon"
                            type="submit"><i class="fa fa-check" aria-hidden="true"></i></button>
                        </div>
                      </td>
                      <td class="text-center">
                        <div class="col-xs-9">
                          <input name="2bottom" class="form-control input-lg" type="text" maxlength="2"
                            value="<?php echo $lotto['2bottom'] ?>" onkeypress="return isNumber(event)"
                            onclick="show_icon('<?php echo '2bottom_'.$lotto['id'] ?>')"
                            onfocusout="hide_icon('<?php echo '2bottom_'.$lotto['id'] ?>')">
                        </div>
                        <div class="col-xs-3">
                          <button id="<?php echo '2bottom_'.$lotto['id'] ?>" class="btn btn-success btn-lg hide-icon"
                            type="submit"><i class="fa fa-check" aria-hidden="true"></i></button>
                        </div>
                      </td>
                      <td class="text-center">
                        <div class="col-xs-9">
                          <input name="3top" class="form-control input-lg" type="text" maxlength="3"
                            value="<?php echo $lotto['3top'] ?>"
                            onclick="show_icon('<?php echo '3top_'.$lotto['id'] ?>')"
                            onfocusout="hide_icon('<?php echo '3top_'.$lotto['id'] ?>')">
                        </div>
                        <div class="col-xs-3">
                          <button id="<?php echo '3top_'.$lotto['id'] ?>" class="btn btn-success btn-lg hide-icon"
                            type="submit"><i class="fa fa-check" aria-hidden="true"></i></button>
                        </div>
                      </td>
                ...
                <div class="form-group">
                  <div class="row">
                    ...
                    <div class="col-md-3 col-sm-9 col-xs-9">
                      <input type="date" name="name" required="required" class="form-control pull-left input-lg"
                        id="datepicker">
                    </div>
                  </div>
                </div>
