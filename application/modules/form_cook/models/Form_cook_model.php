...
class Form_cook_model extends CI_Model {
	...
	{
		$this->db->trans_start();
		$this->db->insert('form_cook',$input);
	...
	public function insert_filename($input)
	{
		...
		$this->db->trans_start();
		$this->db->insert('form_cook_attachment',$input);
		$this->db->trans_complete();
	...
	public function get_data($research_id)
	{
		$this->db->where('research_id',$research_id)
			->join('user','user.user_id=form_cook.user_id');
		$query1 = $this->db->get('form_cook');
		$query = $query1->result_array();

		foreach ($query as $key => $value) {
			$this->db->where('record_id',$value['id']);
			$query2 = $this->db->get('form_cook_attachment');
